...
from ray.data.block import UserDefinedFunction, Schema
...
class Project(AbstractMap, LogicalOperatorSupportsPredicatePassThrough):
    ...
    def _infer_expr_type(self, expr, input_schema: "Schema"):
        ...
        if isinstance(expr, AliasExpr):
            ...
            op = expr.op.value
            if op in ["add", "sub", "mul", "div", "floordiv"]:
                if pa.types.is_integer(left_type) and pa.types.is_integer(right_type):
                    return pa.int64()
                return pa.float64()
            elif op in ["eq", "ne", "lt", "le", "gt", "ge", "and", "or"]:
                return pa.bool_()
